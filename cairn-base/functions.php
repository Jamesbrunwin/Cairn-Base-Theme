...
add_action('init', 'cairn_init');
add_action('wp_enqueue_scripts', 'cairn_enqueue_styles');
add_action('wp_enqueue_scripts', 'cairn_enqueue_scripts');
add_action('admin_enqueue_scripts', 'cairn_admin_enqueue_scripts');
add_action('init', 'cairn_add_theme_support');
add_action('init', 'cairn_register_nav_menus');;
 ...
 * @return null
 */
function cairn_admin_enqueue_scripts() {
  if (file_exists(trailingslashit(get_stylesheet_directory()) . 'admin/style.css')) {
    wp_enqueue_style('cairn-admin-style', trailingslashit(get_stylesheet_directory_uri()) . 'admin/style.css', array(), date('Y-m-d-H:i:s', filemtime(trailingslashit(get_stylesheet_directory()) . 'admin/style.css')));
  }
  if (file_exists(trailingslashit(get_stylesheet_directory()) . 'admin/core.js')) {
    wp_enqueue_script('cairn-admin-scripts', get_stylesheet_directory_uri() . '/admin/core.js');
  }
  do_action('cairn_admin_enqueue_styles');
}
 ...
 * @return null
 */
function cairn_enqueue_styles() {
  wp_enqueue_style('cairn-core-style', get_stylesheet_uri(), array(), date('Y-m-d-H:i:s', filemtime(trailingslashit(get_stylesheet_directory()) . 'style.css')));
  if (file_exists(trailingslashit(get_stylesheet_directory()) . 'hacks.css')) {
    wp_enqueue_style('cairn-hacks-css', trailingslashit(get_stylesheet_directory_uri()) . 'hacks.css', array('cairn-core-style'), date('Y-m-d-H:i:s', filemtime(trailingslashit(get_stylesheet_directory()) . 'hacks.css')));
  }
  do_action('cairn_enqueue_styles');
}
 ...
 * @return null
 */
function cairn_enqueue_scripts() {
  if (!is_admin()) { 
  ...
  do_action('cairn_enqueue_scripts');
}
 ...
 * @return null
 */
if (!function_exists('cairn_add_theme_support')) {
  function cairn_add_theme_support() {
 ...
 * @return null
 */
if (!function_exists('cairn_register_nav_menus')) {
  function cairn_register_nav_menus() {
    register_nav_menus(array(
      'main-navigation'   => 'Main navigation',
    ));
  }
  do_action('cairn_register_nav_menus');
}
 ...
 */
if (!function_exists('cairn_get_img_url')) {
  function cairn_get_img_url($image) {
 ...
 * @return null
 */
if (!function_exists('cairn_img_url')) {
  function cairn_img_url($image) {
    echo cairn_get_img_url($image);
  }
}
 ...
 */
if (!function_exists('cairn_get_post_thumbnail_src')) {
  function cairn_get_post_thumbnail_src($post_id = false, $size = 'full') {
    if (!$post_id) {
      $post_id = get_the_ID();
    }
 ...
 * @param string $size size of thumbnail (defaults to 'full')
 */
if (!function_exists('cairn_post_thumbnail_src')) {
  function cairn_post_thumbnail_src($post_id = false, $size = 'full') {
    echo cairn_get_post_thumbnail_src($post_id, $size);
  }
}
